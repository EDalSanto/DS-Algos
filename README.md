...
#### [How to Solve It Book](https://www.amazon.com/How-Solve-Mathematical-Princeton-Science/dp/069111966X)
      ...
      * Examine the solution obtained
        ...
        * In The Classroom
            ...
            * Main Divisions, Main Questions
                ...
                * Carrying out the plan
        * How to Solve It, a Dialog
            * Getting Acquainted
                * Where should I start? => problem statement
                * What can I do? => visualize the problem; do not concern with details for moment
                * What can I gain by doing so? =>
            * Working for Better Understanding
                * Where should I start? => again problem statement, point at which you may lose sight of it without forgetting about it
                * What can I do? => isolate principal parts of the problem
            * Hunting for the Helpful Idea
    * Exercism first 10 problems
        * Find factors of number => for integers from 1..N, N % integer should equal 0
    * Class
...
```
            ...
            * Infix, Prefix, and Postfix Expressions
                * Prefix and Postfix places operators either before or after operands
        ...
        * **Hashing**
            ...
            * [In Ruby](https://launchschool.com/blog/how-the-hash-works-in-ruby) (oldish, see dox LL idea)
                * Ruby under a microscope book for further details ( but again outdated since change in hash infrastructure )
    * Leetcode problems
        ...
        * Trees => data shape that allows us to represent hierarchical relationships
            * Intro
                ...
                * Definition one => a tree consists of a set of nodes and edges that connect pairs of nodes
                    ...
                    * [Image url](https://bradfieldcs.com/algos/trees/introduction/figures/tree-definition.png)
                * Definition two => a tree is either empty or consists of a root and zero or more subtrees, each of which is also a tree
                    ...
                    * [Image](https://bradfieldcs.com/algos/trees/introduction/figures/tree-definition-recursive.png)
            * Representing a Tree
                * Nodes and References representation
                    ...
                    * https://bradfieldcs.com/algos/trees/representing-a-tree/figures/treerecs.png
        ...
        * Graphs
            * Intro
                ...
                * Path => sequence of vertices that are connected by edges
                    * [Example of directed graph](https://bradfieldcs.com/algos/graphs/introduction/figures/digraph.png)
            ...
            * Implementing Breadth First Search
                ...
                * Method
                    * initialize a set of to retain the records of the vertices that have been visited already
                    * Initialize a queue which will contain all the paths from our starting vertex that we have explored
                        * Initialized with a list containing just the starting vertex
                    * Systematically grow paths one a time starting from the path at the front of the queue
                    * once we have popped from our queue a path to continue exploring and retrieved the last vertex visited from the path => retrieve neighbors from graphs, remove these vertices that we know have already been visited, then for each of the remaining unvisited neighbors:
            ...
            * A knight’s tour / Depth First Search
                ...
                * Knight’s tour => N^K problem, where K is the average branching factor for the board
            ...
            * Binary Search Tree
                ...
                * BFS
                    * Frontier => the set of nodes that we know exist but we have not yet explored
                        * Queue
                    * Would never actually do for binary search because doesn’t make use of order of tree / structure
            * Trie
...
* Advanced Graph Search: Uniform Cost Search and A*
    * [Dijkstra’s Algorithm](https://www.youtube.com/watch?v=pVfj6mxhdMw) => iterative algorithm used to determine the shortest path (by weight) from one particular starting node to all other nodes in the graph
        ...
        * priority queue => ensure that as we explore one vertex after another we are always exploring the one with the smallest distance
        * Greedy algorithm because always mark the shortest vertex
            * Make locally optimal choices at each stage with the hope to make globally optimal choice
    ...
    * A*
        * [Intro](https://www.redblobgames.com/pathfinding/a-star/introduction.html) (really nice visuals)
            * [Algorithms](https://www.redblobgames.com/pathfinding/a-star/introduction.html#algorithms)
                * Key idea for all these algos is that we keep track of an expanding ring called the frontier
                ...
                * Dijkstra’s (Uniform Cost Search)
                    * Prioritize which paths to explore => favors paths with low costs
                ...
                * A* => Modified Dijkstra’s algorithm that is optimized for a single destination as Dijkstra’s can find paths to all locations
            ...
            * A* algorithm
                * Uses both actual distance to the start like Dijkstra’s as well as estimated like Greed BSF
                    * Uses sum
            * Implementations
...
* Constraint Satisfaction Problems
    * Ch 5 of Artificial Intelligence: A Modern Approach - Constraint Satisfaction Problems
        * Intro
            ...
            * Constraint satisfaction problems => states and goal test conform to a standard, structured, and very simple representation
            * Search algorithms can be defined that take advantage of the structure of states and use general-purpose rather problem-specific heuristics
        * 5.1 - CSP
        ...
        * 5.2 - Backtracking Search for CSP
            * Commutative => problem where the the order of application of any given set of actions has no effect on the outcome
                * When assigning values to variables, we reach the same partial assignment, regardless of order => all CSP search algorithms generate successors by considering possible assignments for only a single variable at each node in the search tree
                * Restricts number of leaves of d^n
            ...
            * Propagating Information through constraints
                * Forward checking => whenever a variable X is assigned, looks at each unassigned variable Y that is connected to X by a constraint and delete’s from Y’s domain any value that is inconsistent with the value chosen for X
            * Constraint Propagation => general term for propagating implications of a constraint on one variable onto other variables
                * Needs to be done fast otherwise no better than simple search
                * Arc consistency
